...
R  = @(a1,a2,a3,u)  (1+(a2).*(1-N(a1,u))+a3.*u);%R(U)/R(0)
...
fo  = fitoptions('Methods','NonlinearLeastSquares','StartPoint',0.0001*[1 1 1],'Lower',[0 0 0],'Upper',0.05.*[1 1 1],...
    'Robust','Bisquare','TolX',1e-8,'TolFun',1e-8,'MaxIter',15000,'MaxFunEval',15000,'Normalize','on','DiffMinChange',1e-10);
ftH = fittype('1-(1-a1*x)./(1+(a2*a1+a3)*x)','options',fo);
   
ftD = fittype('((a2*(1-exp(-a1.*x))) +a3.*x )./(1+a2*(1-exp(-a1.*x)))+a3.*x ','options',fo);
% 
