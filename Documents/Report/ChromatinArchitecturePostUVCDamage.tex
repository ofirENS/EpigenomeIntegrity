...
\end{eqnarray}
 ...
 From the equation for $h$, we can extract $L_1$ 
 \begin{equation}
 ...
 \end{equation}
 with $\gamma (\theta)$ the chromatin relaxation factor. 
 
 Since $L_1>L_0$ we can set an lower boundary for the expansion factor $R_T=\frac{L_2}{L_0}$
 \begin{equation*}
 L_2\left(\frac{1-h}{\alpha}+\gamma\right)-\frac{L_0}{\alpha}>L_0 \Rightarrow \frac{L_2}{L_0}=R_T>\frac{\alpha+1}{1-h+\alpha\gamma}
 \end{equation*}
 The chromatin relaxation factor is in the range $0\leq\gamma<1$, and so the lower boundary of the expansion factor $R_{Tmin}$ 
 can only assume values in the range 
 \begin{equation*}
 \frac{1+\alpha}{2-h}\leq R_{Tmin} < \frac{\alpha+1}{1-h}
 \end{equation*}
 
 Because $L_1<L_2$, we can find the upper bound of $R_T$
 
 \begin{equation}
 R_T <\frac{1}{1-h+\alpha(\gamma -1)}
 \end{equation}
 Similarly to the lower bound, the upper bound $R_{Tmax}$ is in the range 
 \begin{equation*}
 \frac{1}{1-h}<R_{Tmax}<\frac{1}{1-h-\alpha}
 \end{equation*}
 
The maximal and minimal values of the expansion factor as a function of the chromatin relaxation factor is plotted in figure \ref{fig:expantionFactorBoundaries}, for several values of histone loss
 
\begin{figure}[H]
\includegraphics[width=0.5\linewidth, height=0.3\textheight]{Images/SlidingModel/expantionFactorBoundaries}
\includegraphics[width=0.5\linewidth,height=0.3\textheight]{Images/SlidingModel/relativeSlidingContribution}
\caption{\tiny{\textbf{Left: Boundaries of the expension factor for chromatin relaxation factor ranging from 0 to 1 is plotted for several values of histone loss. Right: slidig fraction out of the ROI radius $L_2$}}}
\label{fig:expantionFactorBoundaries}
\end{figure}
 
 
 The relative sliding distance out of the overall ROI radius can be calculated as 
 \begin{equation*}
 \frac{L_2-L_1}{L_2}=1-\left(\frac{1-h}{\alpha}+\gamma -\frac{1}{R_T\alpha}\right)
 \end{equation*}
 Values of the relative sliding distance are plotted as a function of $\gamma$ in figure \ref{fig:expantionFactorBoundaries} right box
 
 From $h-d$ we also get the relation 
 \begin{equation*}
 h-d = \alpha \left(\gamma -\frac{L_1}{L_2}\right)\geq 0
 \end{equation*}
in addition to the fact $L_1\geq L_0$ we get
\begin{equation*}
1\geq \frac{L_1}{L_2}\geq \frac{L_0}{L_2}\geq \gamma
\end{equation*}
which means that 
 \begin{equation*}
 L_1\geq L_0 \geq \gamma L_2
 \end{equation*}
