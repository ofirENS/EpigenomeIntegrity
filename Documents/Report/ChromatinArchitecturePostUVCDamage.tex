...
\subsection{1D model of nucleosome sliding and chromatin expansion}
...
\subsection{2D model- incorporating experimental observations}
...
\subsection{Material transfer between concentric rings}
...
F(r)(y(r)+1)dr
\end{equation*}
Assuming the relative amount of material in the ROI is $\gamma$ of its initial value, we calculate 
\begin{equation*}
\frac{\int_0^RF(r)(y(r)+1)dr}{\int_0^RF(r)dr} =\gamma
\end{equation*}
with $R$ the radius of the expansion for which $\gamma$ of the initial material remains. 
Plugging-in the function $F(r),y(r)$ , setting $\gamma =0.6$ and performing integration, we obtain a quadratic polynomial in $R$, which its roots are $R_1=2.23, R_2= -2.14$. Taking the positive root $R_1=2.23$ we see that according to the loss function, $y(r)$ a 40\% loss is obtained by expending the ROI to 2.23. However, in practice the ROI expands to $\sqrt{3}$, so we can assume that about 0.5 is unexplained by pushing. 

We further ask at which distance can we equalize the material loss from the ROI, given the loss function observed. That is, we calculate 
\begin{equation*}
