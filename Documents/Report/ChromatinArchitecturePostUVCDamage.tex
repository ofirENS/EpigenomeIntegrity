...
\section{Microscopic Description of DNA and Nucleusome Loss}
 ...
 \end{enumerate}
 With these assumptions and observations, I present a simplified 1D model which accounts for 14\% loss of histones, with no loss of DNA, in a region expanding $\sqrt{3}$ times its initial size.  In this report I show that the extra percentages of loss can be attributed to nucleosome sliding, cause by exposing the damaged DNA (unwrapping it from histones) by repair proteins.
 
 The first principle to take into account is that sliding histones over any point of the DNA displaces that point 150bp left or right, depending on histone sliding direction (Figure \ref{fig:histoneSlidingSingle}). This is because one full turn of the histone is about 150bp long.This displacement is not a displacement of the point on the actual DNA, but caused by the unwinding of the DNA wrapped around the histone by rolling it.
 
  \begin{figure}[H]
  	\centering
  	\includegraphics[width=0.7\linewidth]{histoneSlidingSingle}
  	\caption{\tiny{\textbf{Three time points during a displacement of damage site (red star)  causes a caused by histone rolling. The displacement of the damage site is equivalent to the length of DNA wrapped on a histone. A displacement is relative to some reference point, like the origin, and does not refer to an actual motion on the DNA}}}
  	\label{fig:histoneSlidingSingle}
  \end{figure}
    
 We know that the radius of the repair zone increases $\sqrt{3}$ folds. This means that the right most damage point should be translated $\sqrt{3}$ times to the right (Figure \ref{fig:histoneSlidingMulti}). 
 
 If the packed region of the DNA contains $N$ nucleosomes is $L$ (Fig \ref{fig:histoneSlidingMulti}), then to translate a point $S$, initially at $L$ to $\sqrt{3}L$, we need to displace it a distance equivalent to the total length of linker DNA in $(\sqrt{3}-1)L$. The number of times we have to slide a histone over the point $S$ is thus $(\sqrt{3}-1)N50/150=(\sqrt{3}-1)N/3$.
 The number $(\sqrt{3}-1)N/3$ is also the number of histones transfered from the left to the right hand side of the point $S$ as it is being pushed towards $\sqrt{3}L$.
 In the region $\sqrt{3}L$ we initially have $\sqrt{3}N$ nucleosomes. After the point $S$ has reached $\sqrt{3}L$ the region contains $N-(\sqrt{3}-1)N/3$ nucleosomes. 
 ...
 \begin{equation*}
 \frac{(\sqrt{3}-1)N}{3\sqrt{3}N}=\frac{(\sqrt{3}-1)}{3\sqrt{3}} =0.14
 \end{equation*}
 ...
 We note further that the ratio of the linker DNA to the histone-wrapped DNA plays a roll in this calculation. If this ratio is $\alpha$, then the number of histones transfered from the left to the right hand side of $S$, and thus lost, is:
 $(\sqrt{3}-1)N\alpha$, and the ratio to the initial number of histones is 
 $\frac{(\sqrt{3}-1)\alpha}{\sqrt{3}}=\alpha(1-1/\sqrt{3})$. For example, if instead of 1.5 turns of DNA around a histone we have only one single turn while displacing histones, then $\alpha=0.5$, and accounts for 100 bp translation of damage points while rolling histones over them. Plugging $alpa=0.5$ in the equation above gives 21\% loss of nucleosomes from the ROI.         
 
 The most general form of the equation above is given if the ratio between the final and initial radius is stated as $\beta$. In the situation above we have $\beta=\sqrt{3}L/L = \sqrt{3}$, but generally it depends on the number of damage sites and the radius of the UV beam. Thus the general expression for the loss of histones is given by 
 \begin{equation*}
 (1-1/\beta)\alpha
 \end{equation*}
 This expression allows us to calculate the loss in the case of more damage sites. 
