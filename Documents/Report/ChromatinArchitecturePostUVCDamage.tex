...
If the chromatin to the left of the point $s$ expends, we can assume that, due to crowding, the chromatin to the right of $s$ condenses. In this scenario, the sliding factor $\alpha$ decreases due to the fact that with each nucleosome sliding over the point $s$, the length the point $s$ travels decreases. In the ratio $\frac{\alpha_1}{\alpha_2}$ the value of $\alpha_2$ decreases while $\alpha_1$ remains constant. 

To describe this process, we assume that the condensation of the chromatin to the right of the point $s$ is inversely proportional to the expansion. That is, 
we assume an angle $\theta_3$ which describes the condensation of the chromatin to the right of $s$, such that the ratio 
\begin{equation*}
\frac{\theta_2}{\theta_1}=\frac{\pi/2 -\theta_3}{\pi/2 -\theta_1}
\end{equation*}
is kept. This brings us to 
\begin{equation*}
\theta_3 = \frac{\pi}{2}\frac{\theta_1-\theta_2}{\theta_1}+\theta_2
\end{equation*}
