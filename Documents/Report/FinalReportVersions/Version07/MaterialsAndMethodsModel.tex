...
\begin{document}	
 ...
 \section{Material and methods: Modeling nucleosomes and DNA redistribution following UV damages} 
	...
	In our model we assume that for $T(u)$ damages, the position of nucleosomes along the DNA 15 minutes post UV-C allows repair with no further sliding of nucleosomes out of the DR. Because nucleosomes wrapped by damaged DNA will be forced to displace in order to expose the damage position, an addition of $dT(u)$ damage will change the organization reached for $T(u)$ to a new one allowing repair with no further sliding. 
	
	In the first-order approximation, the expected additional number of nucleosomes affected by $dT(u)$ damages is $(c_nA_0-N(u))dT(u)$. Therefore, we write	
    ...
    The nucleosomes lost by chromatin de-compaction are those translocated along with undamaged DNA in the DR. Therefore     
	\begin{equation}
